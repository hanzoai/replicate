...
func TestEndToEndEncryptedReplicationRoundTrip(t *testing.T) {
	...
	// Restore while the writer is still live, modelling a read replica hydrating
	// from the durable (S3) backup: age-decrypt the destination back into a fresh
	// SQLite database and verify the data survived the encrypt -> S3 -> decrypt
	// round-trip. This is done BEFORE the writer's Close because a clean shutdown
	// runs local retention that prunes the file-backend replica dir (a durable
	// object store is unaffected); the restore path itself is encryption-agnostic.
	...
	// Clean writer shutdown last (runs local retention on the file replica).
