...
name: replicate
description: >-
  Expert knowledge for contributing to Replicate, a standalone disaster recovery
  ...
  management. Use when working with Replicate source code, writing storage
...
metadata:
  author: hanzoai
  version: "1.0"
  repository: https://github.com/hanzoai/replicate
---

# Replicate Agent Skill

Replicate is a standalone disaster recovery tool for SQLite. It runs as a
...
|---------|--------------------------|-------------------------------------------|
| App     | `cmd/replicate/`        | CLI commands, YAML/env config             |
...
3. Manual fix: `replicate reset <db-path>` clears local LTX state and forces fresh snapshot on next sync (database file is not modified)
4. Automatic fix: set `auto-recover: true` on the replica config to auto-reset on LTX errors (disabled by default)
5. Reference: `cmd/replicate/reset.go`, `replica.go` (auto-recover logic), `db.go` (`ResetLocalState`)
...
correctly configured for Replicate development.
