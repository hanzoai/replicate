...
Condensed reference for agents working with SQLite internals in Replicate.
...
- Replicate monitors WAL and converts frames to LTX format
...
the single most important SQLite detail for Replicate development.
...
Replicate maintains a read transaction for consistency:
...
```sql
PRAGMA journal_mode = WAL;         -- Required for Replicate
...
1. **Automatic checkpoint**: Default at 1000 WAL pages; set high threshold to
   prevent interference with Replicate's control
...
Replicate converts WAL frames to LTX:
