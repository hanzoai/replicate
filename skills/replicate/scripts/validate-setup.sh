#!/bin/sh
# validate-setup.sh - Verify Replicate development environment
...
printf "Replicate Development Environment Validation\n"
printf "==============================================\n\n"
...
printf "\nChecking build...\n"
if go build -o /dev/null ./cmd/replicate 2>/dev/null; then
    pass "Project builds successfully"
else
    fail "Project build failed (go build -o /dev/null ./cmd/replicate)"
fi
...
if [ "$FAIL" -gt 0 ]; then
    ...
    printf "\nEnvironment is ready for Replicate development.\n"
    exit 0
fi
